...
The recursive function purpose is to calculate a generation of a L-system. In order to fullfill its purpose the function needs to provide a quiet big amount of templates.
...
As shown in the listing the 

\begin{enumerate}
\item L-system as interface -> as long as some special functions are provided
\item predec und successor -> auhc implizit um das L-system festzulegen
\item output iterator -> so lange die output teratoren eigenschaften erfüllt sind dynamisch an alles übergeben -> solange der typ funktioniert - beispiel commandmappingiterator
\item nullptr wenn man einfach nur L-system berechnen will -> wenn man es von einem bestimmten stand berechnen will kann man auch den startwert geben -> zb speichern in einem string und dann später weiterrechnen -> zwischenspeichern möglich
\item rekursion -> wenn terminal erreicht wird kann man direkt übergeben ansonsten weiter in der rekursion bis man die maximale tiefe erreicht
\end{enumerate}

\subsection{CommandMappingIterator}

\begin{enumerate}
\item Output Iterator funktionen
\item pointer auf turtle, damit man die entsprechenden Daten aufrufen kann 
\item Problem: wenn die turtle nicht mehr funktioniert - pointer verweist auf etwas falsches, können die call aufrufe nicht mehr funktionieren 
\end{enumerate}


\subsection{TurtleGraphic}
Hier erstmal abstraktes interface eklären dann auf beispiel implementierungen eingehen


\subsubsection{TestTurtle}

implementierung vorstellen -> zum einfachen ausgeben von text auf der concolse für aufrufe
