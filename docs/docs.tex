...
-> in figur .. sieht man nun das konzept für eine Cairo turtle ( es fehlen noch weitere funktionen in der implementierung aber dazu später mehr)


erklären was eine surface target etc ist und dann path erklären -> damit kann man dann auf den status schließen und diesen erklären
...
Erkllären warum bounding boxen notwendig sind -> neu eingführte funktion erklräen -> update bdouning box

-> erklären warum reset vorhanden ist <=> man kann mehrfach zeichnen bzw so werden die dateien nciht kombiniert

-> erklären warum nur einen raw pointer -> hat keinen mehrwert, da das aufräumen von cairo gemacht wird und man damit selber keinen mehrwert hat
...
bounding box enthält keine padding berechnung damit dicke der linie auch berücksichtigt wird
configuration momentan statisch im Code
bounding box nur ein workaround
