...
\medskip
\noindent Finaly, there is a conclusion and an outlook for possible future extensions.
...
There is only a limited number of productions in a grammar of a L-system. Each of these productions is choosen deterministic when comparing the nonterminal. The next generation is created by rewriting the current state, when a nonterminal is often in the current state, the same production can be used. If the same production is used muliple times, the resulting generation has a lot of similar objects or more specific the same string muliple times. The repeating data and the recursive calculation can be used to improve the naive idea.
...
This rather ideal example shows how it would be sufficient to save just parts of the resulting string to represent the whole generation of a L-system. This is even for more complex grammars possible because of the disscused repetitions.

\medskip
\noindent With this knowledge multiple improvements can be discussed. The first possible improvement is storing the data not in a plain object, like a string, but instead in a graph.
A graph has several advantages in comparison to the string concept, for example the reduction of needed memory storage. Such a graph can have a structure like this:





The second possible improvment is to neglect the storage of a state at all. Instead of saving the generation of a L-system in a datastructure, the result gets calculated on demand and never stored. This calculation can be done wit a recursive function and returned in pieces. 
