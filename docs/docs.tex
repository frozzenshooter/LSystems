...
\noindent Another possible simplification is to neglect storing the generations of a L-system at all. Instead of saving the generation of a L-system in a datastructure, the result can be calculated on demand and never stored. This calculation can be done with a recursive function, that will return the generation. Clearly an advantage is the reduction of needed storage space, but this reduction is only achived at the cost of recalculation for each generation. Whenever a generation is needed, it is nessecary to calculate again and it is not possible to use a previous result.
 If a generation is needed multiple time, a solution could be to store the result in another object. This also enables a flexible use, because you can either only calculate the generation once and use the result directly, in this context for example to call the turtle commands, or save it for later usage.
...
Details zum aufrufen 
Semantische schnittstelle bei der funktion

-> genauere beschreibunf im kapitel mit der implementierung

-> woher kommt die konfiguration
